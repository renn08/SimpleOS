// lock unlock signal broadcast wait
#include <iostream>
#include "thread.h"
#include <queue>
#include <vector>
...
int input[5][2] = {{785,53},{350,914},{827,567},{302,230},{631,11}};

void print_request(int i, int j) {
    std::cout << "requester " << i << " track " << j << std::endl;
}

void print_service(int i, int j) {
    std::cout << "service requester " << i << " track " << j << std::endl;
}
...
void service() {
    ...
    for (int i = 0; i < num_requester; ++i) {
        ...
        intptr_t id = i;
        thread((thread_startfunc_t) request, (void *) id);
    }
    ...
    while(num_requester_alive > 0 && queue_cap != 0) {
        ...
        while((int)(request_queue.size()) != largest_possible_num_of_request()) {
            ...
            // cout << (largest_possible_num_of_request()) <<endl;
            lpnr_cv.wait(mutex1);
        }
...
int main() {
    ...
    cpu::boot(1, (thread_startfunc_t) service, (void *) 0, false, false, 0);
}
