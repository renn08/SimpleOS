...
int main(int argc, char *argv[]) {
    ...
    assert(!status);
    status = fs_writeblock("user1", "/dir/file4", 0, writedata);
    assert(!status);
    status = fs_create("user1", "/dir/file5", 'f');
    assert(!status);
    status = fs_writeblock("user1", "/dir/file5", 0, writedata);
    assert(!status);
    ...
    status = fs_readblock("user1", "/dir/file5", 0, readdata);
    assert(!status);
    ...
    assert(!status);
    status = fs_delete("user1", "/dir/file3");
    assert(!status);
    status = fs_delete("user1", "/dir/file4");
    assert(!status);
    status = fs_delete("user1", "/dir/file5");
    assert(!status);
